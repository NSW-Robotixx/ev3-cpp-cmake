...
std::future<void> ButtonNotifier::_listener_thread;
...
void ButtonNotifier::start()
{
    if (_exit_thread) {
        _exit_thread = false;
        _listener_thread = std::async(std::launch::async, &ButtonNotifier::Dispatcher, _pinstance.get());
    }
}

void ButtonNotifier::stop()
{
    if (!_exit_thread) {
        _exit_thread = true;
